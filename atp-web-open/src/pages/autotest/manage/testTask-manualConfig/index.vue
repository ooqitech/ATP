<template>
  <d2-container>
      ...
      <div class="handle-box">
        <el-form ref="baseForm" :model="baseForm" label-width="80px" inline>
          ...
          <el-form-item>
            ...
            <el-button type="primary" @click="handleAddRegress()">新增任务</el-button>
          </el-form-item>
        </el-form>
      </div>
      <el-table :data="tableData" border style="width: 100%" ref="multipleTable" :header-cell-style="{color:'black',background:'#eef1f6'}" size="medium">
        ...
        <el-table-column prop="taskType" label="测试范围">
          <template slot-scope="scope">
            ...
            <span v-else-if="scope.row.taskType === 3">CI构建触发</span>
        ...
        <el-table-column prop="successRate" label="最近一次成功率">
          <template slot-scope="scope">
        ...
        <el-table-column prop="lastRunStatus" label="任务状态">
          <template slot-scope="scope">
          ...
          <template slot-scope="scope">
            <el-dropdown>
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item @click.native="handleRun(scope.$index, scope.row)" v-if="scope.row.taskStatus === 1 && scope.row.lastRunStatus !== '运行中'">运行</el-dropdown-item>
                <el-dropdown-item @click.native="handleEdit(scope.$index, scope.row)" v-if="scope.row.taskType !== 3">编辑</el-dropdown-item>
                <el-dropdown-item @click.native="handleDelete(scope.$index, scope.row)">删除</el-dropdown-item>
                <el-dropdown-item @click.native="viewRunHis(scope.$index, scope.row)">查看运行历史</el-dropdown-item>
                <el-dropdown-item @click.native="viewUncoveIntf(scope.$index, scope.row)" v-if="scope.row.taskStatus === 1 && scope.row.taskType === 2">查看未覆盖接口</el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
            <!--<el-tooltip content="运行" placement="top" effect="light">
              <el-button size="mini" type="success" icon="el-icon-video-play" circle
            ...
            <el-tooltip content="查看运行历史" placement="top" effect="light">
              <el-button size="mini" type="info" icon="el-icon-view" circle
                         @click="viewRunHis(scope.$index, scope.row)"></el-button>
            </el-tooltip>
            <el-tooltip content="查看未覆盖接口" placement="top" effect="light">
              <el-button size="mini" type="info" icon="el-icon-zoom-in" circle
                         @click="viewUncoveIntf(scope.$index, scope.row)"
                         v-if="scope.row.taskStatus === 1 && scope.row.taskType === 2">
              </el-button>
            </el-tooltip>-->
          </template>
        </el-table-column>
    ...
    <!--回归测试配置界面-->
    <el-dialog title="回归测试配置" :close-on-click-modal="false" :close-on-press-escape="false" :visible.sync="cfgRegressVisible" width="60%" ref="editDialog">
      <el-form ref="regForm" :model="regForm" label-width="80px">
        <el-form-item label="任务名称" prop="regName" :rules="[{ required: true, message: '任务名称必填', trigger: 'blur' }]">
          <el-input v-model="regForm.regName" placeholder="请输入名称"></el-input>
        </el-form-item>
        <el-form-item label="所属项目" prop="projectId">
          <el-select v-model="regForm.projectId" filterable clearable placeholder="请选择项目" :disabled="isSelect" @change="changeProject">
            <el-option v-for="item in regForm.projectOptions" :key="item.projectId" :label="item.projectName" :value="item.projectId"></el-option>
          </el-select>
          <el-checkbox
            style="margin-left: 30px"
            v-model="regForm.isRelated"
            :disabled="regForm.projectId === '' || regForm.projectId === 0"
            @change="isRelated"
          >是否关联项目下的接口用例</el-checkbox>
        </el-form-item>
        <el-form-item label="用例标签" prop="tagList" :rules="[{ type: 'array',required: true, message: '用例标签至少选择一项', trigger: 'change' }]">
          <el-checkbox-group v-model="regForm.tagList">
            <el-checkbox v-for="(item,index) in regForm.tagOptions" :key="index" :label="item.tagId">{{item.tagName}}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
        <el-form-item label="测试范围" :rules="[{ required: true, message: '测试范围必选', trigger: 'change' }]">
          <el-radio-group v-model="taskType" @change="isChanged">
            <el-radio label="1">人工指定</el-radio>
            <el-radio label="2">基于代码变更</el-radio>
            <el-radio label="3">CI构建触发</el-radio>
          </el-radio-group>
          <!--<el-checkbox style="margin-left: 30px" v-model="regForm.isFilter" :disabled="taskType === '2'" @change="isFiltered">是否根据标签过滤</el-checkbox>-->
        </el-form-item>
        <!--<el-form-item :label="item.label" :prop="'tagOptions.' + index + '.value'"
                              v-for="(item,index) in regForm.tagOptions" :key="index" v-if="regForm.isFilter">
          <el-radio-group v-model="item.value" @change="fetchByTags">
            <el-radio :label="subItem.tagId" v-for="(subItem,index) in item.tagList" :key="index">
              {{subItem.tagName}}
            </el-radio>
          </el-radio-group>
        </el-form-item>-->
        <el-tabs v-model="activeName" type="border-card" v-show="taskType === '1' || taskType === '3'">
          <el-tab-pane label="接口用例" name="first">
            <div class="example_tree">
              <div class="navigation-filter">
                <el-input placeholder="输入关键字搜索接口用例" v-model="filterText"></el-input>
              </div>
              <!--树状结构选择接口用例-->
              <el-scrollbar wrap-class="list-regression" view-class="view-box" :native="false">
                <div class="category">
                  <el-tree
                    :data="intfTreeData"
                    node-key="id"
          ...
          <el-tab-pane label="全链路用例" name="second">
            <div class="example_tree">
              <div class="navigation-filter">
                <el-input placeholder="输入关键字搜索全链路用例" v-model="filterFullText"></el-input>
              </div>
              <!--树状结构-->
              <el-scrollbar wrap-class="list-regression" view-class="view-box" :native="false">
                <div class="category">
        ...
        <el-form v-show="taskType === '2'">
          <el-form ref="changeSystemForm" :model="regForm.changeSystemForm" label-width="80px" inline>
            <el-form-item label="变更系统" prop="changeSystemId" :rules="[{ required: true, message: '变更系统必选', trigger: 'change' }]">
              <el-select v-model="regForm.changeSystemForm.changeSystemId" filterable placeholder="请选择项目" @change="changeSystem">
                <el-option v-for="item in regForm.systemOptions" :key="item.systemId" :label="item.label" :value="item.systemId"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="git URL" prop="gitUrl" :rules="[{ required: true, message: 'git URL必填', trigger: 'blur' }]">
              <el-input v-model="regForm.changeSystemForm.gitUrl" placeholder="请输入git URL"></el-input>
            </el-form-item>
            <el-form-item>
              <el-button @click="getBranch" type="primary" plain>获取分支</el-button>
            </el-form-item>
            <el-form ref="changeSystemForm1" :model="regForm.changeSystemForm">
              <el-form-item label="分支名称" prop="changeBranchName" :rules="[{ required: true, message: '分支必选', trigger: 'change' }]">
                <el-select v-model="regForm.changeSystemForm.changeBranchName" filterable placeholder="请选择分支名称">
                  <el-option v-for="item in regForm.branchOptions" :key="item" :label="item" :value="item"></el-option>
                </el-select>
              </el-form-item>
              <el-form-item label="起始commitId" prop="commitStartId" :rules="[{ required: true, message: '起始id必填', trigger: 'blur' }]">
                <el-input v-model="regForm.changeSystemForm.commitStartId" placeholder="请输入起始id"></el-input>
              </el-form-item>
              <el-form-item label="结束commitId" prop="commitEndId" :rules="[{ required: true, message: '结束id必填', trigger: 'blur' }]">
                <el-input v-model="regForm.changeSystemForm.commitEndId" placeholder="请输入结束id"></el-input>
              </el-form-item>
              <el-form-item>
                <el-button @click="handAddChangeSystem()" type="primary" plain>添 加</el-button>
              </el-form-item>
            </el-form>
          </el-form>
          <el-table
            :data="regForm.changeSystemListForView"
            border
            style="width: 100%"
            ref="multipleTable"
            :header-cell-style="{color:'black',background:'#eef1f6'}"
          >
            <el-table-column prop="changeSystemId" label="变更系统" sortable></el-table-column>
            <el-table-column prop="gitUrl" label="git URL"></el-table-column>
            <el-table-column prop="changeBranchName" label="分支名称"></el-table-column>
            <el-table-column prop="commitStartId" label="起始id"></el-table-column>
            <el-table-column prop="commitEndId" label="结束id"></el-table-column>
            <el-table-column label="操作" fixed="right" width="100">
              <template slot-scope="scope">
                <el-button type="text" @click="handleEditChangeSys(scope.$index, scope.row)">编辑</el-button>
                <el-button type="text" @click="handleDeleteChangeSyS(scope.$index, scope.row)">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
        </el-form>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="cfgRegressVisible = false">取 消</el-button>
        <el-button type="primary" @click="saveEditReg('regForm')">确 定</el-button>
      </span>
    </el-dialog>
    <!-- 删除提示框 -->
    ...
    <el-dialog title="运行测试" :visible.sync="runVisible" width="45%">
      <el-form ref="runForm" :model="runForm" label-width="100px">
        ...
        <el-form-item label="是否检查服务器时间" prop="isCheckServerTime">
          <el-select v-model="runForm.isCheckServerTime" placeholder="请选择">
            <el-option v-for="item in runForm.checkOptions" :key="item.index" :label="item.name" :value="item.value"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
...
import {
  ...
  listForTask,
  reCollectTaskResult
} from '@/api/autotest/manage/testTask-manualConfig'
  ...
  methods: {
        ...
        if (valid) {
          ...
          if (this.failedRetry) {
            requestParams = {
              ...
              preCheckServerTime: this.runForm.isCheckServerTime
            }
          } else {
    ...
    getTaskList(page, num, id, keyWords) {
      return new Promise((resolve, reject) => {
        let searchParams = keyWords ? { pageNo: page, pageSize: num, companyId: id, keyWords: keyWords } : { pageNo: page, pageSize: num, companyId: id }
        listTask(searchParams).then(res => {
          if (res.code === '000') {
