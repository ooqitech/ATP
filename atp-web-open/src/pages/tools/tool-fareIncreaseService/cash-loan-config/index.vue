...
<script>
import { queryFareIncreaseServiceConfig, setCashLoanConfig } from '@/api/tools/tool-fareIncreaseService'
import { isJsonString } from '@/libs/common'
var env = 'ALIUAT'

export default {
  name: 'cash-loan-config',
  props: {
    mappingList: {
      type: Object,
      default: null
  ...
  methods: {
      ...
      queryFareIncreaseServiceConfig({ env: env }).then(res => {
        ...
        if (JSON.stringify(correctObj) !== '{}') {
          for (let n in correctObj) {
            if (n === 'feeRateStrategyOfCashLoan') {
              // 现金贷按期配置加价策略
              let id = 1
              correctObj.feeRateStrategyOfCashLoan.forEach(item => {
              ...
              this.fareIncreaseServiceConfig.data = correctObj.feeRateStrategyOfCashLoan
    ...
    submitData(data) {
      return new Promise((resolve, reject) => {
        setCashLoanConfig(data).then(res => {
