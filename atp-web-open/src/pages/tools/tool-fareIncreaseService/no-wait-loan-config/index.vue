  ...
  <div>
    ...
    >
      <el-form :model="fareIncreaseServiceConfig.addForm" labelWidth="110px" labelPosition="right" size="mini" ref="fareIncreaseServiceConfigAddFrom">
        ...
        <el-row :gutter="5">
          <el-col :span="12">
            <el-form-item label="用户评级" prop="userLevel" :rules="[{ required: true, message: '用户评级必填', trigger: 'blur' }]">
...
<script>
import { queryFareIncreaseServiceConfig, setNoWaitLoanConfig } from '@/api/tools/tool-fareIncreaseService'
import { isJsonString } from '@/libs/common'
var env = 'ALIUAT'

export default {
  name: 'no-wait-loan-config',
  props: {
    mappingList: {
      type: Object,
      default: null
  ...
  methods: {
      ...
      queryFareIncreaseServiceConfig({ env: env }).then(res => {
        ...
        if (JSON.stringify(correctObj) !== '{}') {
          for (let n in correctObj) {
            if (n === 'feeRateStrategyOfNoWaitLoan') {
              // 现金贷按期配置加价策略
              let id = 1
              correctObj.feeRateStrategyOfNoWaitLoan.forEach(item => {
              ...
              this.fareIncreaseServiceConfig.data = correctObj.feeRateStrategyOfNoWaitLoan
    ...
    handAdd() {
      ...
      this.fareIncreaseServiceConfig.addForm.scene = '医美扫码@不等贷订单'
    ...
    saveEdit(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          if (this.fareIncreaseServiceConfig.addForm.scene !== '医美扫码@不等贷订单') {
            this.$message.warning('场景不正确,只能配置医美扫码@不等贷订单')
            return false
          }
    ...
    submitData(data) {
      return new Promise((resolve, reject) => {
        setNoWaitLoanConfig(data).then(res => {
