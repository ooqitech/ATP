...
from atp.httprunner import (exceptions, loader, logger, parser, report, runner,
                            utils, validator)
...
class HttpRunner(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.log_path = self.kwargs.pop("log_path", None)

        if self.log_path:
            # 增加一个日志handler，写入run_{report_id}.log
            self.handler = logging.FileHandler(self.log_path)
            fmt = '[%(asctime)s] [%(levelname)-5s] %(message)s'
            format_str = logging.Formatter(fmt)
            self.handler.setFormatter(format_str)
            logger.add_handler(self.handler)

    # def __del__(self):
    #     logger.remove_handler(self.handler)
    ...
    def parse_tests(self, testcases, variables_mapping=None):
        ...
        for testcase in testcases:
            ...
            for parameter_mapping in cartesian_product_parameters_list:
                ...
                temp_variable_mapping = {}
                for raw_config_variable in raw_config_variables:
                    parsed_config_variable = parser.parse_data(
                        raw_config_variable,
                        temp_variable_mapping,
                        self.project_mapping["debugtalk"]["functions"]
                    )
                    parsed_config_variables.append(parsed_config_variable)
                    temp_variable_mapping.update(parsed_config_variable)
    ...
    def gen_html_report(self, html_report_name=None, html_report_template=None, html_report_dir=None):
        ...
        Args:
            ...
            html_report_dir(str): report dir
