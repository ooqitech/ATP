...
# from atp.utils.custom import setup_disconf_operation
...
def setup_hook_prepare_kwargs(request):
    if request["method"] == "POST":
        ...
        if content_type and "json_str" in request:
            if content_type.startswith("application/json") and isinstance(request["json_str"], str):
                request["json"] = json.loads(request["json_str"])
                request.pop("json_str")
